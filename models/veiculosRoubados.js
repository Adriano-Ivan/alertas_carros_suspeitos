...
class veiculosRoubados {
  ...
  async buscarPorPlaca(placa) {
    const sql = `SELECT v.dono, v.placa, s.status, n.nivel_urgencia, v.local_roubo, v.local_alerta FROM veiculos_roubados AS v INNER JOIN status AS s ON s.id = v.statusID INNER JOIN nivel_urgencia AS n ON v.nivel_urgenciaID = n.id WHERE placa = '${placa}'`;
