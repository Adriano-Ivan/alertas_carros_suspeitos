...
class VeiculosRoubados {
  ...
  async pegarDadosAlerta(id_zona) {
    const sql =
      "SELECT v.id,v.placa,v.local_alerta,v.alertado,v.id_zona, 'roubado' as tipo,DATE_FORMAT(DATE(momento_alerta),'%d/%m/%Y') as data, time(v.momento_alerta) as hora FROM veiculos_roubados AS v INNER JOIN status AS s ON s.id = v.statusID WHERE v.alertado = false and s.status != 'Resolvido' and v.id_zona = ? ORDER BY momento_alerta DESC;";
    const dados = await queryPromise2(id_zona, sql);
    return [].concat(dados);
  }
