...
class VeiculosInfracao {
  ...
  async pegarDadosPorId(id) {
    const sql =
      "SELECT v.id,v.dono, v.placa,v.id_ultimo_editor, s.status, n.nivel_urgencia,v.gravidade_infracao,v.local_alerta,DATE_FORMAT(DATE(momento_alerta), '%d/%m/%Y') as data, time(v.momento_alerta) as hora FROM veiculos_infracao AS v INNER JOIN status AS s ON s.id = v.statusID INNER JOIN nivel_urgencia AS n ON v.nivel_urgenciaID = n.id WHERE v.id = ? ORDER BY momento_alerta DESC;";
